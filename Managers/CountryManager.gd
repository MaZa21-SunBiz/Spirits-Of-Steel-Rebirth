...
func get_country_used_manpower(country_obj: CountryData) -> int:
	...
	for troop in TroopManager.get_troops_for_country(country_obj.country_name):
		for div in troop.stored_divisions:
			total_used += _get_manpower_from_template(div.type, country_obj)

	# 2. Ongoing Training (Already using templates, but cleaned up)
	for training in country_obj.ongoing_training:
		total_used += (
			training.divisions_count * _get_manpower_from_template(training.division_type, country_obj)
		)
...
func _get_manpower_from_template(type: String, country_obj: CountryData) -> int:
	var fallback = country_obj.templates.get("infantry", {"manpower": 10000})
	return country_obj.templates.get(type, fallback).get("manpower", 10000)
