...
@onready var flag_rect = main_vbox.get_node("HBoxContainer/nation_flag")
@onready var header_label = main_vbox.get_node("HBoxContainer/Label")

# Category Tabs
@onready var military_grid = main_vbox.get_node("TabContainer/MILITARY/VBoxContainer2/ScrollContainer/VBoxContainer")
@onready var economy_grid = main_vbox.get_node("TabContainer/ECONOMY/VBoxContainer2/ScrollContainer/VBoxContainer")
@onready var country_grid = main_vbox.get_node("TabContainer/COUNTRY/VBoxContainer2/ScrollContainer/VBoxContainer")
@onready var releasables_grid = main_vbox.get_node("TabContainer/RELEASABLES/VBoxContainer2/ScrollContainer/VBoxContainer")
@onready var economic_status_val = main_vbox.get_node("TabContainer/ECONOMY/VBoxContainer/HBoxContainer/Values")
@onready var army_logistics_val = main_vbox.get_node("TabContainer/MILITARY/VBoxContainer/HBoxContainer2/Values")

# Data
@onready var current_country: CountryData
var _update_timer: float = 0.0
#endregion

func _process(delta: float) -> void:
	# current_country = CountryManager.player_country
	_update_timer += delta
...
func open_logistics(country: String):
	_populate_military()
	_populate_economy()
	_populate_country()
	_populate_releasables(country)
...
func _populate_releasables(player_country: String) -> void:
	...
	if releasables.is_empty():
		...
		releasables_grid.add_child(lbl)
	else:
		for releasables_grid in releasables:
			_add_releasable_option(current_country.country_name)
#endregion
...
func _update_law_buttons_visuals() -> void:
	if not military_grid: return
	# NOTE(soi): why
	for btn in military_grid.get_children():
...
func _add_releasable_option(country_id: String) -> void:
	...
	releasables_grid.add_child(btn_panel)
...
func _add_law_option(
	...
	military_grid.add_child(btn_panel)
